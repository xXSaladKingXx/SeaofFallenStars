...
public static class SettlementStatsCache
{
    ...
    public class SettlementComputedStats
    {
        ...
        // Additional demographic aggregations
        public Dictionary<string, int> populationByReligion = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, int> populationByLanguage = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        ...
        string parentTerms)
    {
        ...
        if (childIds.Count > 0)
        {
            ...
            {
                bool childIsCapital = !string.IsNullOrWhiteSpace(capitalId) && string.Equals(childId, capitalId, StringComparison.OrdinalIgnoreCase);
                float incomeTax = 0f;
                float troopTax = 0f;
                string terms = null;
                if (childIsCapital)
                ...
                MergeCounts(stats.populationByReligion, childStats.populationByReligion);
                MergeCounts(stats.populationByLanguage, childStats.populationByLanguage);
    ...
    private static void AddLocalDemographics(SettlementComputedStats stats, SettlementInfoData data)
    {
        ...
        // Religion distribution (simple per-settlement assignment)
        if (!string.IsNullOrWhiteSpace(data?.cultural?.religion))
        {
            AddCount(stats.populationByReligion, data.cultural.religion.Trim(), stats.localPopulation);
        }
        else
        {
            AddCount(stats.populationByReligion, "Unspecified", stats.localPopulation);
        }
        // Languages distribution (each language counts the whole population)
        if (data?.cultural?.languages != null && data.cultural.languages.Length > 0)
        {
            foreach (string lang in data.cultural.languages)
            {
                if (!string.IsNullOrWhiteSpace(lang))
                    AddCount(stats.populationByLanguage, lang.Trim(), stats.localPopulation);
            }
        }
        else
        {
            AddCount(stats.populationByLanguage, "Unspecified", stats.localPopulation);
        }
    }
    ...
    private static void AddPercentDistribution(Dictionary<string, int> dict, int totalPop, List<PercentEntry> dist)
    {
        ...
        for (int i = 0; i < dist.Count; i++)
        {
            ...
            int count = (i == dist.Count - 1) ? Mathf.Max(0, totalPop - assigned) : Mathf.RoundToInt(totalPop * pct);
            assigned += count;
